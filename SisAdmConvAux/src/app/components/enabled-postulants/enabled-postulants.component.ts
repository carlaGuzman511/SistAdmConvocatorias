...
@Component({
  selector: "app-enabled-postulants",
  templateUrl: "./enabled-postulants.component.html",
  styleUrls: ["./enabled-postulants.component.css"],
})
/**
  This class contains the component that show the enabled postulants
  @class EnabledPostulantsComponent
*/
export class EnabledPostulantsComponent implements OnInit {
  private user: User;
  ...
  */
  private searchFuntion(indice: number): void {
    this.enable.idannouncement = this.announcements[indice].idannouncement;
    this.enable.text = this.searchForm.controls.searchtext.value.toLowerCase();
    this.searchEnabledPostulants(indice);
  }
  /**
    This function searches the enabled postulants
    @method searchEnabledPostulants()
  */
  private searchEnabledPostulants(indice: number): void {
    this.enable.idannouncement = this.announcements[indice].idannouncement;
          ...
          .subscribe((events: Event[]) => {
            ...
            if (today > enabledPublication) {
              if (today <= publicationOfResults) {
                this.apiService
                  .post("/logbook/searchByText/status", this.enable)
                  .subscribe(
                    (response: LogBook[]) => {
                      this.logBooks = response;
                      this.generatePostulantAnnouncement();
                    },
                    (error: HttpErrorResponse) => {
                      this.showMessage.showError(
                        "No se encontraron postulantes habilitados para esta convocatoria"
                      );
                    }
                  );
              } else {
  ...
  */
  ...
  /**
   This function shows the person postulation for each announcement
    @method generatePostulantAnnouncement()
  */
  private generatePostulantAnnouncement(): void {
    const logbookS = [];
    const creditCards = [];
    for (const logbook of this.logBooks) {
      if (!creditCards.includes(logbook.label.postulantes.person.ci)) {
        logbookS.push(logbook);
        creditCards.push(logbook.label.postulantes.person.ci);
      }
    }
  ...
  */
  private printTable(): void {
    ...
    const header = function (data) {
      ...
      doc.text("Postulantes Habilitados", data.settings.margin.right * 6, 90);
